class SearchLinesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @search_line.save
        ...
        format.json { render action: 'show', status: :created, location: @search_line }
      else
  ...
  def destroy
    @search_line.destroy
    respond_to do |format|
      format.html { redirect_to search_lines_url }
      format.json { head :no_content }
    end
  end
  ...
  def search_line_params
    params.require(:search_line).permit(:name, :user_name)
  end
end
